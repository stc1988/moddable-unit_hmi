...
- `onChanged` receives the same sample when the raw value changes by more than `deadband`. Assigning a callback starts polling; assigning `null` stops it.
